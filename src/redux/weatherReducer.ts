  ...
  switch (action.type) {
    ...
    case 'WA/WR/GET_FORECAST':
      return {
        ...
        ...state.oneDayForecast.forecast.forecastday[0] = { ...state.oneDayForecast.forecast.forecastday[0], date: action.date, day: action.day },
      };
...
export const getWeather = (city: string | null, lat?: number | null, long?: number | null): ThunkType => async (dispatch) => {
  const response = await weatherAPI.getWeather(city, lat, long);
  const response2 = await weatherAPI.getForecastForOneDay(city, lat, long);
