    ...
    return (
        <div>
            ...
            <Layout className={s.container_page}>
                ...
                <Content >
                    <div>{location.tz_id + ','} {location.region + ','} {location.country + ','} сlouds: {current.cloud + '%, '} {location.localtime}</div>
                    <div> {current.temp_c + 'C,'} wind: {current.wind_kph + 'kph'}, pressure: {current.pressure_mb + 'hpa'} </div>
                    <div> Geo coords: {location.lat + ', ' + location.lon} </div>
                </Content>
                <Divider className={s.divider} />
                <div><h2>One Day Forecast: </h2></div>
                <Content>
                    <div>{location.tz_id + ','} {location.region + ','} {location.country + ','} {oneDayForecast.day.condition.text} </div>
                    <div> min_temperature: {oneDayForecast.day.mintemp_c + 'C,'} max_temperature: {oneDayForecast.day.maxtemp_c + 'C,'} averege_temperature: {oneDayForecast.day.avgtemp_c + 'C.'} </div>
                    <div> Geo coords: {location.lat + ', ' + location.lon} </div>
                </Content>
            </Layout>
        </div>
